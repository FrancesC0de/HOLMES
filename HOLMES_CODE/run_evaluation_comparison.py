...
def generate_heatmaps(model_name, num_classes, test_images_path, meronyms, idx_to_class, checkpoint, printout=False, num_exp=None, size=(224, 224)):
...
def run_evaluation_comparison(CONFIG, HOL_MER_DICT, CKP_PATH, TEST_IMGS_PATH, MODEL, verbose=0, num_exp=50):
  ...
  curves_json = CONFIG + '_curves.json'
  ...
  for HOLONYM in HOL_MER_DICT.keys():
    ...
    if HOLONYM not in curves_dict:
        ...
        heatmaps = generate_heatmaps(MODEL, NUM_CLASSES, test_images_path, MERONYMS, IDX_TO_CLASS, checkpoint=CHECKPOINT, num_exp=num_exp)
...
if __name__ == '__main__':
  ...
  run_evaluation_comparison(CONFIG, HOL_MER_DICT, CKP_PATH, TEST_IMGS_PATH, MODEL, verbose=VERBOSE, num_exp=NUM_EXP)
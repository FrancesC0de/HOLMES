...
from netlib.transformers import get_transformer_model, deit_ft, swin, get_swin_model
...
def load_best_model(model_name, NUM_CLASSES, save_file_name, freeze=True):
  ...
  if model_name == 'vgg16':
    ...
    model = swin(NUM_CLASSES, freeze=freeze)
  else:
...
def get_model_by_name(model_name, hooks=False):
    if model_name == 'vgg16':
      ...
      model = get_swin_model() if hooks == False else swin(None, edit=False).cuda().eval()
    else:
      raise Exception('Model {} not supported.'.format(model_name))
